...
def visualize_connectome(adjacency_matrix, src, subject, subjects_dir, edge_threshold=None, edge_cmap='viridis',
                         symmetric_cmap=False, linewidth=6.0, node_size=3.0, colorbar=True, colorbar_height=0.5,
    ...
    c = plotting.view_connectome(adjacency_matrix, node_coords, edge_threshold=edge_threshold, edge_cmap=edge_cmap,
                                 ...
                                 colorbar=colorbar, colorbar_height=colorbar_height,
...
def fdr_control(d_all, k, alpha, method='arithmetic'):
    """FDR control based on BY procedure

            Parameters
            ----------
            d_all:  ndarray of shape (n_samples, n_sources, n_sources)
                deviance matrix
        ...
        """
    if d_all.ndim == 2:
        d_all = d_all.reshape((1,) + d_all.shape)

    if method == 'arithmetic':
        d = np.mean(d_all, axis=0)
    elif method == 'median':
        import statistics
        n_ = d_all.shape[1]
        d = np.zeros((n_, n_))
        for i, j in itertools.product(range(n_), repeat=2):
            d[i, j] = statistics.median(dev_all[:, i, j])
    ...
    j_val_low = np.zeros_like(d, dtype=float)
    j_val_up = np.zeros_like(d, dtype=float)
    ...
    for i in range(len(gc_idx)):
        ...
        df = k[row, col]
        dev_low = np.sqrt(d_all[:, row, col])
        dev_low_ = np.mean(dev_low, axis=0)
        dev_low_ = dev_low_ ** 2
        dev_low_ -= df
        non_centrality_low = dev_low_ if dev_low_ > 0 else 0

        dev_up_ = np.minimum((np.mean(d_all[:, row, col], axis=0) - df) * (1 + 1 / df),
                             np.mean(np.sqrt(d_all[:, row, col]), axis=0) ** 2)
        non_centrality_up = dev_up_ if dev_up_ > 0 else 0

        j_val_low[row, col] = 1 - alpha_bar - ncx2.cdf(1 - chi2.isf(1 - alpha_bar, k[row, col]), k[row, col],
                                                       non_centrality_low)
        j_val_up[row, col] = 1 - alpha_bar - ncx2.cdf(1 - chi2.isf(1 - alpha_bar, k[row, col]), k[row, col],
                                                      non_centrality_up)

    return j_val_low, j_val_up
...
def plot_connectivity_circle_(con, node_names, indices=None, n_lines=None,
